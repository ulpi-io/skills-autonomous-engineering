...
function runnerCommands(idPrefix, platforms = ['claude', 'codex'], className = '') {
  const definitions = {
    ...
    codex: {
      label: 'Codex via skills.sh',
      mark: 'X',
      command: codexPipelineRun,
      note: 'Run the skills.sh version today. The native Codex plugin remains explicitly preview.',
    },
  };
...
export function renderPluginsHub({ plugins }) {
  ...
  <section><div class="wrap"><div class="install-callout panel"><div><p class="lab"><span class="mk">▸</span> works today</p><h2>Install the contracts anywhere.</h2><p>skills.sh is the common denominator. Plugin pages describe the provider-native layers on top.</p></div><div>${codeRow(universalInstall, 'plugins-universal')}</div></div>${runnerCommands('plugins-pipeline', ['claude', 'codex'], 'runner-block-after-install')}</div></section>`;
...
export function renderCodexPlugin({ skills }) {
  ...
  <section id="available-now"><div class="wrap"><div class="section-head reveal"><p class="lab"><span class="mk">▸</span> available now</p><h2>Install the skills, then run the pipeline.</h2><p>Codex supports the SKILL.md anatomy through skills.sh today; this is not presented as the native plugin.</p></div>${codeRow(universalInstall, 'codex-universal')}${runnerCommands('codex-pipeline', ['codex'], 'runner-block-after-install')}<div class="chip-grid">${skills.map((skill) => `<a href="${pathFor(route, `skills/${skill.slug}`)}">$${esc(skill.slug)}</a>`).join('')}</div></div></section>
