...
function runnerCommands(idPrefix, platforms = ['claude', 'codex'], className = '') {
  const definitions = {
    ...
    codex: {
      ...
      note: 'Install the plugin, start a new session, then run. Missing capability and verification failures stay explicit.',
    },
  };
...
export function renderHome({ groups, skills, plugins }) {
  ...
  <section>
    <div class="wrap install-callout panel reveal"><div><p class="lab"><span class="mk">▸</span> universal skill install</p><h2>Start with all eighteen.</h2><p>Install individual skills through skills.sh, or choose the full Claude Code or Codex plugin for a provider-native surface.</p></div><div>${codeRow(universalInstall, 'home-install')}<a class="text-link" href="${pathFor(route, 'plugins')}">Compare plugin distributions →</a></div></div>
  </section>`;
  return layout({
    route,
    title: site.name,
...
export function renderPluginsHub({ plugins }) {
  ...
  return layout({ route, active: 'plugins', title: 'Claude Code and Codex plugins', description: 'Compare the available Claude Code and Codex plugins, universal skills installation, hooks, context, orchestration, and exact invocation commands.', body });
}
...
export function renderClaudePlugin({ skills }) {
  ...
  <section><div class="wrap"><div class="callout"><span class="stamp stamp-ok">available</span><div><h2>Native on both platforms.</h2><p>Claude Code keeps the canonical slash-command and Workflow surface. Codex ships its own adapters, coordinator, executor, new-session discovery, and install verification.</p></div><a class="btn ghost" href="${pathFor(route, 'plugins/codex')}">Compare Codex →</a></div></div></section>`;
...
export function renderCodexPlugin({ skills }) {
  ...
  <section class="page-hero plugin-hero"><div class="wrap page-hero-grid"><div><div class="hero-meta"><p class="lab"><span class="mk">X</span> Codex plugin</p>${statusBadge('available', 'Available from source · v0.1.0')}</div><h1>Codex-native.<br>Evidence-gated.</h1><p class="lede">The native plugin ships 18 qualified adapters, a deterministic coordinator, pinned codex exec contracts, durable checkpoints, new-session discovery, and reproducible install verification.</p><div class="hero-actions"><a class="btn" href="#install">Install plugin →</a><a class="btn ghost" href="${site.repository}/tree/main/.codex-plugin">Inspect manifest ↗</a></div></div><div class="release-panel panel"><span class="stamp stamp-ok">available</span><p class="lab">SOURCE ARTIFACT / v0.1.0</p><strong>catalog_count = 18</strong><ul><li>18 sealed adapters</li><li>pinned executor</li><li>durable checkpoints</li><li>isolated install smoke</li><li>new-session discovery</li></ul></div></div></section>
  <section id="install"><div class="wrap"><div class="section-head reveal"><p class="lab"><span class="mk">▸</span> source install</p><h2>Build the artifact. Add it. Verify it.</h2><p>From a source checkout, the packager writes only to the output directory and prints a stable digest. Register that local marketplace, verify it, add the plugin, then verify all 18 skills before starting a new Codex session.</p></div>${codexInstallCommands.map((command, index) => codeRow(command, `codex-install-${index + 1}`)).join('')}${runnerCommands('codex-pipeline', ['codex'], 'runner-block-after-install')}<div class="chip-grid">${skills.map((skill) => `<a href="${pathFor(route, `skills/${skill.slug}`)}">${esc(skill.slug)}</a>`).join('')}</div></div></section>
  <section><div class="wrap"><div class="section-head reveal"><p class="lab"><span class="mk">▸</span> implemented control plane</p><h2>The availability claim has executable proof.</h2><p>Each capability maps to concrete runtime code and deterministic verification. A missing gate remains gateNotRun; it never becomes optimistic copy.</p></div><ol class="roadmap-list">${codexCapabilities.map((item, index) => `<li><span>${String(index + 1).padStart(2, '0')}</span><p>${esc(item)}</p></li>`).join('')}</ol></div></section>
  ...
  <section><div class="wrap"><div class="callout"><span class="stamp stamp-ok">source install</span><div><h2>Installed explicitly. Verified deliberately.</h2><p>The plugin is available as a reproducible source-built marketplace artifact. The gated live smoke checks install, discovery, and invocation against the real Codex CLI without fabricating green.</p></div><a class="btn ghost" href="${sourceFor('scripts/package-codex-plugin.mjs')}">Inspect packager ↗</a></div></div></section>`;
  return layout({ route, active: 'plugins', title: 'Codex plugin', description: 'Install the available v0.1.0 Codex plugin with 18 qualified adapters, deterministic orchestration, new-session discovery, and reproducible verification.', body });
}
