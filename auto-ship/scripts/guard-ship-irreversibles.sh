...
def _cmd_str(v):
    ...
    # ["bash","-lc","git push --force"]); shlex.quote each token so element boundaries survive
    # re-tokenization instead of word-splitting into a broken parse. Any other type → "" (fail-open).
    if isinstance(v, str):
        return v
